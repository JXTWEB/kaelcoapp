...
$result2 = mysqli_query($db,"SELECT TELLNAME FROM ftellers WHERE TELINI = '$initfbil_collectedby'  ");
...
$name_coll1 = $tel2['TELLNAME'];
 ...
 $initfbil_collectedby  = substr($rowmonth['LE'.$x] ,0,3);
$result = mysqli_query($db,"SELECT TELLNAME FROM ftellers WHERE TELINI = '$initfbil_postedby'  ");
while ($tel = mysqli_fetch_array($result))
{

$name_pos = $tel['TELLNAME'];
}
$result1 = mysqli_query($db,"SELECT TELLNAME FROM ftellers WHERE TELINI = '$initfbil_collectedby'  ");
